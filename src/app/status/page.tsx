...
export default function StatusPage() {
  ...
  async function check() {
    ...
    if (s.status === "fulfilled") {
      setStatus(s.value as FlightStatusInfo | null);
    } else {
      setStatus(null);
    }

    if (l.status === "fulfilled" && l.value?.flights) {
      setLive((l.value.flights as LiveFlightInfo[]).slice(0, 10));
    } else {
      setLive(null);
    }
      ...
      <div className="container-app max-w-3xl">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="text-sm text-[var(--text-secondary)] mb-8">
            Track flight status and view aircraft in the air. Search by flight number or browse live traffic.
          </p>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          className="surface-card rounded-[28px] p-5 mb-8"
        >
          <div className="flex gap-3">
            ...
            <button onClick={check} disabled={!flightNum || loading}
              className="self-end px-5 py-2.5 bg-[var(--accent-cta)] text-[var(--text-inverse)] font-semibold rounded-full hover:opacity-90 disabled:opacity-30 transition-opacity flex items-center gap-2 text-sm"
            >
        ...
        {status ? (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
            <div className="surface-card rounded-[28px] p-5">
              ...
              {status.position && (status.position.lat || status.position.lon) ? (
                <div className="grid grid-cols-3 gap-3 mb-4">
                  <div className="text-center p-3 bg-[var(--bg-base)] rounded-[18px]">
              ...
              {status.origin_country ? (
                <div className="text-xs text-[var(--text-secondary)]">
                  Origin: {status.origin_country} · Last contact: {status.last_contact ? new Date(status.last_contact).toLocaleString() : "N/A"}
                </div>
              ) : null}
            </div>
            ...
            <Link
              ...
              className="block w-full py-3 bg-[var(--accent-primary-dim)] border border-[var(--accent-cta)]/20 text-[var(--accent-cta)] font-semibold rounded-full text-center text-sm hover:bg-[var(--accent-cta)] hover:text-[var(--text-inverse)] transition-colors"
            >
        ...
        {live && live.length > 0 ? (
          <div className="mt-8">
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto">
              {live.map((f: LiveFlightInfo, i: number) => (
                <div key={i} className="flex items-center justify-between p-2.5 rounded-[18px] bg-[var(--bg-base)] border border-[var(--border-default)] text-xs">
